function plot_convergence_curves_1(results_list, save_dir, fontSizeOptions)
% plot_convergence_curves - Plots convergence curves for multiple algorithms (Enhanced Version).
%
% [Final Version]: 1. Corrected the algorithm classification list to ensure exact name matching.
%                  2. Added visual separators in the legend for clearer grouping.
%
% Inputs:
...
if nargin < 3 || isempty(fontSizeOptions)
    fontSizeOptions = struct('title',16, 'labels',24, 'ticks',16, 'legend',18);
end

% --- [CORE MODIFICATION 1: Update algorithm classification list for exact name matching] ---
% Update the algorithm names here based on your debug output.
% Please double-check that the strings here exactly match your `results_list{i}.name`!
...
h_central = []; 
h_federated = []; 
h_uncategorized = []; % Add a handle to store uncategorized algorithms
...
for i = 1:length(results_list)
    ...
    % --- Store handles by category and print debug information ---
    ...
    elseif ismember(res.name, federated_algs)
        ...
        h_uncategorized(end+1) = p; % Collect handles for uncategorized algorithms
...
% If there are any uncategorized algorithms, display them in the legend for debugging
...
% --- Create a grouped legend with separators (for the second plot) ---
hold on;
